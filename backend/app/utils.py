...
def criar_evento_e_enviar_alerta(id_pet, titulo, data_evento, hora_evento=None, descricao=''):
    ...
    query_pref = """
        SELECT t.receber_notificacoes 
        FROM tutores t
        JOIN pets p ON p.id_tutor = t.id_tutor
        WHERE p.id_pet = %s
    """
    pref = consultar_db(query_pref, (id_pet,), one=True)

    destinatarios = [tutor['email'] for tutor in tutores]
    nome_pet = pet['nome_pet']
    titulo_completo = f"Alerta para {nome_pet}: {titulo}"

    if pref and pref.get('receber_notificacoes'):
        try:
