...
def criar_evento_e_enviar_alerta(id_pet, titulo, data_evento, hora_evento=None, descricao=''):
    ...
    "                   FROM pets " \
    "                   WHERE id_pet = %s", (id_pet,))
    if not pet:
        return jsonify({"error": "Alerta não enviado, pet não encontrado"}), 404
    
    query_tutores = """ SELECT t.email FROM tutores t JOIN tutor_pet tp ON t.id_tutor = tp.id_tutor WHERE tp.id_pet = %s"""

    tutores = consultar_db(query_tutores, (id_pet,))
    if not tutores:
        return jsonify({"error": "Alerta não enviado, nenhum tutor associado ao pet"}), 404
    ...
    if hora_evento:
        if isinstance(hora_evento, str):
            hora_evento = time.fromisoformat(hora_evento)

        data_inicio = datetime.combine(data_evento, hora_evento)
        ...
        datas_formatadas = f"{data_evento.strftime('%Y%m%d')}/{data_fim.strftime('%Y%m%d')}"
...
def tutor_e_pets(id_tutor):
    query_tutor = """SELECT *
               FROM tutores
               WHERE id_tutor = %s"""
    tutor = consultar_db(query_tutor, (id_tutor,), one=False)
    if not tutor:
        return jsonify({"error": "Tutor não encontrado"}), 404
...
def deletar_pet(id_pet, id_tutor):
    query = """DELETE FROM pets p 
                USING tutor_pet tp
                WHERE p.id_pet = tp.id_pet AND p.id_pet = %s AND tp.id_tutor = %s"""
...
@app.route('/api/pets/<int:id_pet>/convidar-tutor', methods=['POST'])
def convidar_tutor_para_pet(id_pet):
    dados = request.json
    email_tutor_convidado = dados.get('email')
    id_tutor_remetente = dados.get('id_tutor_remetente')

    if not email_tutor_convidado or not id_tutor_remetente:
        return jsonify({"error": "Email do tutor convidado e ID do tutor remetente são obrigatórios."}), 400

    tutor_convidado = consultar_db("SELECT * FROM tutores WHERE email = %s", (email_tutor_convidado,))
    if not tutor_convidado:
        return jsonify({"error": "Tutor com o email fornecido não encontrado."}), 404
    
    pet = consultar_db("SELECT * FROM pets WHERE id_pet = %s", (id_pet), one=True)
    if not pet:
        return jsonify({"error": "Pet não encontrado."}), 404
    
    tutor_remetente = consultar_db("SELECT * FROM tutores WHERE id_tutor = %s", (id_tutor_remetente,))
    ...
    if error:
        if 'unique constraint' in error:
            return jsonify({"error": "O tutor já está associado a este pet."}), 400
...
def adicionar_vacina_por_pet(id_pet):
    ...
    if proxima_dose:
        criar_evento_e_enviar_alerta(
            id_pet=id_pet,
            titulo=f"Próxima dose da vacina {dados.get('nome_vacina')}",
            data_evento=proxima_dose,
            descricao=f"Lembrete para a próxima dose da vacina {dados.get('nome_vacina')} para {dados.get('nome_pet')}."
        )
...
def criar_consulta_pet(id_pet):
    ...
    nome_clinica = dados.get('local_consulta')
    nome_veterinario = dados.get('nome_veterinario')
...
def anexar_arquivo(id_consulta):
    ...
    consulta = consultar_db(query, (id_consulta,))
    if not consulta:
...
def criar_remedio_pet(id_pet):
    ...
    if error:
        return jsonify({"error": f"Erro ao criar remédio: {error}"}), 500
    
    if proxima_dose:
        criar_evento_e_enviar_alerta(
            id_pet=id_pet,
            titulo=f"Próxima dose do remédio {dados.get('nome_remedio')}",
            data_evento=proxima_dose,
            descricao=f"Lembrete para a próxima dose do remédio {dados.get('nome_remedio')} para {dados.get('nome_pet')}."
        )

    return jsonify({"message": "Remédio criado com sucesso."}), 201
...
def criar_clinica():
    ...
    query = """INSERT INTO clinicas (nome_clinica, endereco, telefone, email)
               VALUES (%s, %s, %s, %s)"""
    _, error = executar_db(query, (nome_clinica, endereco, telefone, email))
    if error:
        return jsonify({"error": f"Erro ao criar clínica: {error}"}), 500
...
def criar_produto_pet(id_pet):
    ...
    preco_compra = dados.get('preco_compr')
    loja = dados.get('loja')
    observacoes = dados.get('observacoes')
    ...
    query = """INSERT INTO produtos (id_pet, nome_produto, categoria, quantidade, consumo_medio, data_compra, preco_compra, loja)
               VALUES (%s, %s, %s, %s, %s, %s, %s)"""
    _, error = executar_db(query, (id_pet, nome_produto, categoria, quantidade, consumo_medio, data_compra, preco_compra, loja))
    if error:
...
def editar_produto(id_compra):
    ...
    query = """UPDATE produtos 
               ...
               WHERE id_compra = %s"""
    _, error = executar_db(query, (nome_produto, categoria, quantidade, consumo_medio, data_compra, preco_compra, loja, observacoes, id_compra))

    if error:
        return jsonify({"error": f"Erro ao atualizar produto: {error}"}), 500
    return jsonify({"message": "Produto atualizado com sucesso."}), 200

@app.route('/api/produtos/<int:id_compra>/consumo', methods=['POST'])
def registrar_consumo_produto(id_compra):
    ...
    produto = consultar_db("SELECT * FROM produtos WHERE id_compra = %s", (id_compra,))
    if not produto:
    ...
    if dias_restantes <= 7:
        ...
        criar_evento_e_enviar_alerta(
            ...
            descricao=f"O estoque do produto {produto['nome_produto']} está baixo, lembrete para comprar mais para {dados.get('nome_pet')}."
        )
...
def esqueci_senha():
    ...
    if tutor:
        ...
        try:
            ...
            return jsonify({"error": f"Erro ao enviar email de redefinição: {str(e)}"}), 500
        pass
